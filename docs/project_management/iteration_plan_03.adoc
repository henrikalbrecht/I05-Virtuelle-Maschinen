...
| PRB01: Aufgabenverteilung | fixed (siehe Assessment)
...
| PRB02: Git Workflow | fixed  (siehe Assessment)| Um die Arbeit an einzelnen Funktionalitäten in den Pull requests besser unterscheiden zu können, ist eine Arbeit mit branches notwendig. Die Arbeit mit branches muss bei einem weiteren Workshop während eines Team-Meetings vermittelt werden.

| PRB03: Nutzung von Google Docs | new | Für die Erstellung des Glossars wurde durch die Verantwortlichen ein Google Docs Dokument angelegt, was Mehrarbeit zur Folge hat, da die Daten im Anschluss in das entsprechende Dokument auf Github überführt werden müssen. Es gilt Einigung darüber zu erreichen, dass Arbeit direkt über die Entwicklungsumgebung an den Github-Dokumenten durchgeführt wird, um effizient zu bleiben.

| PRB04: Statusaktualisierung Github Issues | new | Die in Git angelegten Issues, welche die Verteilung der Aufgaben an alle Teammitglieder darstellt, sind dafür vorgesehen, von dem entsprechenden Bearbeiter in den Status "In progress" gesetzt zu werden, sobald die Arbeit daran beginnt. Diese Regel wird aktuell ungenügend beachtet und muss in einem zukünftigen Team-Meeting erneut vermittelt werden.

| PRB05: Unzureichende Arbeitsergebnisse | new | Die Arbeitsergebnisse eines Teammitgliedes werden konstant als unzureichend bewertet. Die negativen Auswirkungen auf die Motivation anderer Teammitglieder, der erhöhte Zeitaufwand und die Mehrbelastung durch Kompensation durch das übrige Team sind deutlich erkennbar. Das Problem muss in geeigneter Weise schnellsmöglich behandelt werden.
|===
...
[%header, cols="1,3"]
|===
| Assessment Ziel | Problemlösung zu __PRB01__
...
[%header, cols="1,3"]
|===
| Assessment Ziel | Problemlösung zu __PRB02__
...
* Das Problem PRB05 wurde während der Retrospektive am 18.12.2023 angesprochen und nach Lösungen gesucht, die Situation zu verbessern. Die Projektleitung erörtert mit Hilfestellung des Coaches schnellstens eine mögliche Strategie für den worst-case.
