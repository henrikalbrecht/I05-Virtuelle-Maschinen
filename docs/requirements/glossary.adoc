...
| Docker-Container | Enthält, im vergleich zur VM, nur alle nötigen Resourcen, zur Ausführung einer bestimmten Software. Lässt sich leicht als Datei transportieren und installieren. | 
...
| Frameworks |   Ist ein Programmiergerüst und dient als Hilfestellung zum Programmieren. Es umfasst Komponenten, Laufzeitumgebungen und Bibliotheken.|
...
| PostgreSQL Datenbank  | Eine Open Source Datenbank, die sich stark an den SQL-Standart hält. |
| VM Pool | (Umfang) bezieht sich auf eine Sammlung von virtuellen Maschinen |
| Django | Django ist ein leistungsstarkes |
...
| ORM | Objektrelationale Abbildung (object-relational mapping) | Technik der Softwareentwicklung, in der durch ein mit objektorientierten Programmiersprache geschriebenes Anwendungsprogramm, Objekte in einer relationalen Datenbank ablegen kann. 
